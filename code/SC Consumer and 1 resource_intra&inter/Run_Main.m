...
%clear
tic
consumer_diversity = 6;
simulation_step_number = 2e8;           % simulation steps for SSA
...
select_index = [1:1e3, 2e3:1e4:length(time_ssa)];
ts=time_ssa(select_index);
cs=consumer_ssa(select_index, :)' ;
rs=resource_ssa(select_index, :)';
ssa=[ts',cs',rs'];
ct=time_ode(:,:);
co=evolution_ode(:,1:6);
cr=evolution_ode(:,end);
ode=[ct, co ,cr];

figure
hold on
plot(ssa(1:3:end,1),ssa(1:3:end,2:8))
plot(ode(:,1),ode(:,2:8))
plot(ts, rs,'r','linewidth',3)
plot(time_ode, evolution_ode(:,1:6))
